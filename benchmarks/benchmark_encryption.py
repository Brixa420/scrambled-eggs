...
def run_benchmark(config: Dict[str, Any], data_size: int, iterations: int = 10) -> Dict[str, Any]:
    ...
    def get_stats(times):
        return {
            ...
            "throughput": (
                (data_size / 1024 / 1024) / statistics.median(times)
                if statistics.median(times) > 0
                else 0
            ),  # MB/s
        }
...
def main():
    ...
    for config in CONFIGURATIONS:
        ...
        print(
            f"Min Layers: {config['min_layers']}, Max Layers: {config['max_layers']}, Target Time: {config['target_time_ms']}ms"
        )
        print("=" * 40)
        ...
        for size in tqdm(DATA_SIZES, desc="Data Sizes"):
            ...
            print(
                f"  Encryption: {result['encrypt']['mean']:.2f} ± {result['encrypt']['stddev']:.2f} ms"
            )
            print(
                f"  Decryption: {result['decrypt']['mean']:.2f} ± {result['decrypt']['stddev']:.2f} ms"
            )
