...
class SecurityMiddleware:
    ...
    def _is_suspicious_user_agent(self) -> bool:
        ...
        suspicious_patterns = [
            'nmap', 'nikto', 'sqlmap', 'w3af', 'nessus', 'metasploit', 'burp',
            'dirbuster', 'nikto', 'acunetix', 'paros', 'skipfish', 'wpscan',
            'sql injection', 'xss', 'lfi', 'rfi', 'exec', 'eval', 'union select',
            '--', ';--', '1=1', '1=0', ' or ', ' and ', '/*', '*/',
            'script>', '<script', 'onerror=', 'onload=', 'onmouseover=',
            'javascript:', 'vbscript:', 'data:text/html', 'base64',
        ]
    ...
    def _detect_attack_patterns(self) -> bool:
        ...
        if request.form:
            ...
            if any(re.search(pattern, form_data, re.IGNORECASE | re.DOTALL) for pattern in all_patterns):
                return True
        ...
        if request.is_json:
            try:
                ...
                if any(re.search(pattern, json_data, re.IGNORECASE | re.DOTALL) for pattern in all_patterns):
                    return True
            except:
                pass
