...
class ClippyAI:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        if not self.config.api_key:
            raise ValueError("API key is required. Set CLIPPY_AI_API_KEY environment variable or pass api_key.")
    ...
    def analyze_radicalization(self, text: str) -> int:
        ...
        response = self._call_ai([{"role": "user", "content": prompt}], temperature=0.3, max_tokens=1)
    ...
    def generate_response(self, user_input: str, radicalization_level: Optional[int] = None) -> str:
        ...
        # Generate response
        messages = [{"role": "system", "content": context}] + self.conversation_history[-10:]  # Keep last 10 messages for context
