...
from typing import Dict, List, Optional, Tuple, Union, AsyncGenerator, Callable, Any, Awaitable
...
class FileTransferManager:
    ...
    async def prepare_file_for_sending(self, 
                                    file_path: Union[str, Path],
                                    recipient_public_key: bytes,
                                    custom_metadata: Optional[Dict[str, Any]] = None) -> Tuple[FileMetadata, bytes]:
        """
    ...
    async def prepare_file_receiving(self, 
                                   metadata: FileMetadata,
                                   save_directory: Union[str, Path]) -> str:
        """
    ...
    async def process_incoming_chunk(self, 
                                   file_id: str, 
                                   chunk_index: int, 
        ...
        try:
            ...
            if (chunk_index < transfer.metadata.chunk_count - 1 and 
                len(chunk_data) != transfer.metadata.chunk_size):
            ...
            if transfer.metadata.is_encrypted and transfer.metadata.encryption_key:
                chunk_data = self._decrypt_chunk(
                    chunk_data,
                    transfer.metadata.encryption_key,
                    transfer.metadata.iv,
                    chunk_index
                )
    ...
    def register_progress_callback(self, file_id: str, callback: Callable[[str, float], Awaitable[None]]):
        """Register a progress callback for a file transfer."""
        self.progress_callbacks[file_id] = callback
    
    def register_completion_callback(self, file_id: str, callback: Callable[[str, Path], Awaitable[None]]):
        """Register a completion callback for a file transfer."""
        self.completion_callbacks[file_id] = callback
    
    def register_error_callback(self, file_id: str, callback: Callable[[str, str], Awaitable[None]]):
