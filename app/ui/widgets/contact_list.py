...
from PySide6.QtWidgets import (
    ...
    QVBoxLayout, QPushButton, QMenu, QFrame, QStyledItemDelegate
)
    ...
    def setup_ui(self):
        ...
        if self.contact.avatar:
            ...
            colors = ['#ff9a9e', '#fad0c4', '#a1c4fd', '#c2e9fb', '#d4fc79', 
                     '#96e6a1', '#84fab0', '#8fd3f4', '#a6c1ee', '#fbc2eb']
...
class ContactListWidget(QListWidget):
    ...
    def update_contacts(self, contacts: List[Contact]):
        ...
        sorted_contacts = sorted(online, key=lambda x: x.name.lower()) + \
                         sorted(offline, key=lambda x: x.name.lower())
    ...
    def filter_contacts(self, text: str):
        ...
        for i in range(self.count()):
            ...
            widget = self.itemWidget(item)
            if widget:
                matches = (text in widget.contact.name.lower() or 
                          text in widget.contact.id.lower())
