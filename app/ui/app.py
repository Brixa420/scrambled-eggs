...
from app.ui.components.message_status import create_message_status_indicator
from app.ui.components.file_transfer import create_file_transfer_manager
from app.ui.components.group_chat import create_group_chat_manager
...
class ScrambledEggsApp:
    ...
    def _create_header(self):
        ...
        # App title
        title = widgets.HTML(
            "<h1 style='margin: 0; padding: 10px;'>🔐 Scrambled Eggs</h1>"
        )
        ...
        header = widgets.HBox([
            title,
            widgets.HBox([self.connection_status], layout={'width': '100%', 'justify_content': 'flex-end'})
    ...
    def _init_contacts_tab(self):
        ...
        self.contact_manager = create_contact_manager(
            self.p2p_manager,
            on_contact_select=self._on_contact_selected
        )
        
        # Contacts tab layout
        self.contacts_tab = widgets.VBox([
    ...
    def _init_security_tab(self):
        ...
        security_tabs.children = [
            self.security_dashboard.dashboard,
            self.file_transfer.manager
        ]
        ...
        with self.chat_display:
            for msg in messages:
                ...
                if msg.get('is_own', False):
                    ...
                    print(f"  <div style='background-color: #e3f2fd; display: inline-block; padding: 8px 12px; border-radius: 15px; max-width: 70%;'>")
                    ...
                    print(f"  <div><strong>{sender}</strong> <span style='font-size: 0.8em; color: #666;'>{time}</span></div>")
                    print(f"  <div style='background-color: #f5f5f5; display: inline-block; padding: 8px 12px; border-radius: 15px; max-width: 70%;'>")
                    print(f"    {msg['content']}")
                    print("  </div>")
                    print("</div>")
