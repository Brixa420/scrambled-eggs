...
class ContactManager:
    ...
    def _update_contact_list(self, filter_text: str = ''):
        ...
        filtered_contacts = {
            contact_id: info for contact_id, info in self.contacts.items()
            if (filter_text in contact_id.lower() or 
                filter_text in info.get('name', '').lower())
        }
        ...
        self.contact_list.options = [
            (f"🟢 {info.get('name', contact_id)}" if info.get('is_online', False) 
             else f"⚪ {info.get('name', contact_id)}", contact_id)
