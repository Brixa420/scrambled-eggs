...
class FileTransferManager:
    ...
    def _init_ui(self):
        ...
        self.send_button = widgets.Button(
            ...
            icon='upload',
            tooltip='Send a file to a contact'
        )
    ...
    def _on_send_click(self, _):
        ...
        contact_options = [(f"{c.get('name', cid)} ({cid})", cid) 
                          for cid, c in contacts.items()]
        
        contact_dropdown = widgets.Dropdown(
            options=contact_options,
            description='To:',
            disabled=not contact_options
        )
    ...
    def _add_transfer(self, transfer: FileTransfer):
        ...
        peer_label = widgets.HTML(f"To: {transfer.peer_id}" if transfer.direction == 'send' 
    ...
    def _remove_transfer(self, transfer_id: str):
        ...
        if transfer_id in self.transfer_widgets:
            ...
            self.transfer_list.children = tuple(
                c for c in self.transfer_list.children 
                if c != transfer_widget
            )
    ...
    def _update_transfer(self, transfer: FileTransfer):
        ...
        widgets['open_button'].disabled = transfer.status != 'completed' or transfer.direction != 'receive'
    ...
    def _show_file_offer_dialog(self, transfer: FileTransfer):
        ...
        dialog = widgets.VBox([
            ...
            widgets.HTML(f"<p><b>{transfer.filename}</b> ({self._format_size(transfer.size)})</p>"),
