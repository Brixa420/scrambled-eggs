...
class CryptoEngine:
    ...
    def encrypt_symmetric(self, data: bytes, key_id: str) -> bytes:
        ...
        # Encrypt
        cipher = Cipher(
            self.SYMMETRIC_ALGORITHM(key),
            self.SYMMETRIC_MODE(iv)
        )
    ...
    def decrypt_asymmetric(self, data: bytes, key_id: str) -> bytes:
        ...
        private_key = load_pem_private_key(
            self._key_pairs[key_id].private_key,
            password=None
        )
    ...
    def import_key_pair(self, key_id: str, public_key: bytes, private_key: Optional[bytes] = None) -> None:
        """
        Import a key pair.
    ...
    def encrypt_message(self, message: str, recipient_public_key: bytes) -> Dict[str, Any]:
        ...
        # Encrypt
        cipher = Cipher(
            self.SYMMETRIC_ALGORITHM(session_key),
            self.SYMMETRIC_MODE(iv)
        )
        encryptor = cipher.encryptor()
