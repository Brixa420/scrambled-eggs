...
class Gate:
    ...
    def _chacha20(self, data: bytes, operation: str) -> bytes:
        ...
        cipher = Cipher(
            algorithms.ChaCha20(self.key, self.iv),
            mode=None,
            backend=default_backend()
        )
...
class GateSystem:
    ...
    def get_random_gates(self, count: int, gate_type: Optional[GateType] = None) -> List[Gate]:
        ...
        candidates = [
            ...
            if gate_type is None or gate.gate_type == gate_type
        ]
        return random.sample(candidates, min(count, len(candidates)))
    
    def process_through_gates(self, data: bytes, gate_ids: List[int], operation: str = 'encrypt') -> bytes:
    ...
    def get_gate_metrics(self) -> Dict:
        ...
        active_gates = [
            ...
            if (now - g.last_accessed) < timedelta(hours=1)
        ]
        
        return {
            'total_gates': len(self.gates),
            'active_gates': len(active_gates),
            'avg_access_count': sum(g.access_count for g in self.gates.values()) / len(self.gates) if self.gates else 0,
            'gate_types': {
                gt.name: len([g for g in self.gates.values() if g.gate_type == gt])
                for gt in GateType
            },
            'algorithms': {
                'AES-256-CBC': len([g for g in self.gates.values() if g.encryption_algorithm == 'AES-256-CBC']),
                'AES-256-GCM': len([g for g in self.gates.values() if g.encryption_algorithm == 'AES-256-GCM']),
                'ChaCha20': len([g for g in self.gates.values() if g.encryption_algorithm == 'ChaCha20']),
                'Blowfish': len([g for g in self.gates.values() if g.encryption_algorithm == 'Blowfish']),
            },
    ...
    def get_health_status(self) -> Dict:
        ...
        return {
            ...
            'health_score': min(100, len(active_gates) / len(self.gates) * 100) if self.gates else 0,
