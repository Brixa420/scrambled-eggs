...
class SelfModifyingEncryption:
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized SelfModifyingEncryption with {security_level.name} security level"
        )
    ...
    def _adapt_security_level(self, event: Optional[Dict[str, Any]] = None) -> None:
        ...
        if event:
            ...
            self.security_events = [
                e for e in self.security_events if current_time - e.get("timestamp", 0) <= 3600
            ]
        ...
        if recent_events > 10:
            ...
            new_level = SecurityLevel(
                max(self.security_level.value - 1, SecurityLevel.MEDIUM.value)
            )
        else:
            # No change needed
            return
    ...
    def encrypt(self, plaintext: bytes, additional_data: Optional[bytes] = None) -> Dict[str, Any]:
        ...
        if self.current_profile.algorithm == "AES":
            if self.current_profile.mode == "CBC":
                ...
                # Create cipher
                cipher = Cipher(algorithms.AES(self._current_key), modes.CBC(iv))
                ...
                cipher = Cipher(algorithms.AES(self._current_key), modes.GCM(iv))
                encryptor = cipher.encryptor()
        ...
        return self.crypto_engine.encrypt_message(
            plaintext.decode("utf-8"), self.crypto_engine.get_public_key()
        )

    def decrypt(
        self, encrypted_data: Dict[str, Any], additional_data: Optional[bytes] = None
    ) -> bytes:
        """
        ...
        try:
            ...
            if "GCM" in algorithm:
                ...
                cipher = Cipher(algorithms.AES(self._current_key), modes.GCM(iv, tag))
    ...
    def get_security_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "next_rekey_in": max(
                0, self.last_rekey_time + self.current_profile.rekey_interval - time.time()
            ),
