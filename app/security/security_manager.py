...
class SecurityManager:
    ...
    def _update_threat_level(self, event: SecurityEvent) -> None:
        ...
        self.ai_orchestrator.analyze_gate_performance(
            gate_id=0, metrics={"threat_score": threat_score_increase}  # Global threat level
        )
    ...
    def _handle_unauthorized_access(self, event: SecurityEvent) -> None:
        ...
        self._log_audit_event(
            ...
            "blocked",
            {"user_id": user_id, "source_ip": source_ip, "details": event.details},
        )
    ...
    def _handle_rate_limit(self, event: SecurityEvent) -> None:
        ...
        if len(self.rate_limits[source]) > max_requests:
            ...
            logger.warning(
                f"Rate limit exceeded for {source}, blacklisted for {blacklist_duration} seconds"
            )
            self._log_audit_event(
                "rate_limit_exceeded",
                "blocked",
    ...
    def _handle_anomaly(self, event: SecurityEvent) -> None:
        ...
        if gate_id is not None:
            self.ai_orchestrator.analyze_gate_performance(
                gate_id=gate_id, metrics=event.details.get("metrics", {})
            )
    ...
    def _handle_key_rotation(self, event: SecurityEvent) -> None:
        ...
        self._log_audit_event(
            "key_rotation",
            "success",
            {"key_type": key_type, "initiated_by": event.details.get("initiated_by", "system")},
        )
    ...
    def increase_security_level(self) -> None:
        ...
        self._log_audit_event(
            "security_level_change", "success", {"new_level": self.security_level.name}
        )
    ...
    def _rotate_affected_keys(self, identifier: str) -> None:
        ...
        self._log_audit_event(
            "key_rotation", "initiated", {"reason": "security_event", "identifier": identifier}
        )
    ...
    def _rotate_all_keys(self) -> None:
        ...
        self._log_audit_event("full_key_rotation", "initiated", {"reason": "security_policy"})
    ...
    def _rotate_keys_of_type(self, key_type: str) -> None:
        ...
        self._log_audit_event(
            "key_rotation", "initiated", {"key_type": key_type, "reason": "security_policy"}
        )
    ...
    def _rotate_gate(self, gate_id: int) -> None:
        ...
        self._log_audit_event(
            "gate_rotation", "initiated", {"gate_id": gate_id, "reason": "anomaly_detected"}
        )
    ...
    def _send_security_alert(self, subject: str, message: str) -> None:
        ...
        self._log_audit_event("security_alert", "sent", {"subject": subject, "message": message})
        ...
        if len(self.rate_limits[identifier]) >= max_requests:
            ...
            self.handle_event(
                SecurityEvent(
                    ...
                    details={"count": len(self.rate_limits[identifier]), "limit": max_requests},
                )
            )
            return False
    ...
    def save_state(self, filepath: str) -> None:
        ...
        state = {
            ...
            "locked_accounts": {k: v.isoformat() for k, v in self.locked_accounts.items()},
    ...
    @classmethod
    def load_state(
        cls, filepath: str, crypto_engine: CryptoEngine, gate_system: GateSystem
    ) -> "SecurityManager":
        ...
        try:
            ...
            manager.locked_accounts = {
                k: datetime.fromisoformat(v) for k, v in state.get("locked_accounts", {}).items()
            }
            ...
            manager.security_events = [
                SecurityEvent.from_dict(e) for e in state.get("security_events", [])
            ]
