...
from Cryptodome.Hash import SHA3_512, HMAC
    ...
    def decrypt(self, encrypted_data: bytes, metadata: Dict) -> bytes:
        ...
        for layer_info in reversed(metadata['layers']):
            try:
                ...
                logger.warning(f"Decryption failed at layer {layer_info.get('layer_number', 'unknown')}: {e}")
...
class ScrambledEggsLayeredEncryption:
    ...
    def __init__(self, min_layers: int = 3, max_layers: int = 10):
        ...
        Args:
            ...
            max_layers: Maximum number of encryption layers to apply
        """
        from .scrambled_eggs_encryption import ScrambledEggsEncryption, AIParams
    ...
    def encrypt(self, data: bytes) -> Tuple[bytes, Dict]:
        ...
        enc_session_key, encrypted_data, hmac_value = self.core_encryption.hybrid_encrypt(data, public_key)
    ...
    def decrypt(self, encrypted_data: bytes, metadata: Dict) -> bytes:
        ...
        try:
            ...
            return self.core_encryption.hybrid_decrypt(enc_session_key, iv_tag_ciphertext, hmac_value)
