...
class ClippyEncryption:
    ...
    def _generate_keys(self):
        ...
        self._private_key = ec.generate_private_key(
            ec.SECP384R1(), default_backend()  # Using P-384 for ECDH
        )
    ...
    def _derive_key(
        self, password: bytes, salt: bytes, key_length: int, iterations: int = 100000
    ) -> bytes:
        """
    ...
    def _hmac_sign(self, data: bytes, key: bytes, algorithm: str = "sha512") -> bytes:
        ...
        h = hmac_lib.HMAC(key, getattr(hashes, algorithm.upper())(), backend=default_backend())
        h.update(data)
        return h.finalize()

    def _hmac_verify(
        self, data: bytes, signature: bytes, key: bytes, algorithm: str = "sha512"
    ) -> bool:
    ...
    def _encrypt_chunk(
        self, chunk: bytes, key: bytes, layer_config: LayerConfig
    ) -> Tuple[bytes, Dict]:
        ...
        if layer_config.layer_type == EncryptionLayer.AES_256_CBC:
            ...
            return encrypted, {"iv": iv, "hmac": hmac_value, "original_length": len(chunk)}
            ...
            return encrypted, {"iv": iv, "tag": encryptor.tag, "original_length": len(chunk)}
            ...
            poly = hmac_lib.HMAC(poly_key, hashes.Poly1305(), backend=default_backend())
            poly.update(encrypted)
            tag = poly.finalize()

            return encrypted, {"iv": iv, "tag": tag, "original_length": len(chunk)}

        else:
            raise ValueError(f"Unsupported encryption layer: {layer_config.layer_type}")

    def _decrypt_chunk(
        self, chunk: bytes, key: bytes, layer_config: LayerConfig, metadata: Dict
    ) -> bytes:
        ...
        if layer_config.layer_type == EncryptionLayer.AES_256_CBC:
            ...
            if layer_config.use_hmac and "hmac" in metadata:
                if not self._hmac_verify(
                    metadata["iv"] + chunk, metadata["hmac"], key, layer_config.hmac_algorithm
                ):
                    raise ValueError("HMAC verification failed")

            # Decrypt
            cipher = Cipher(
                algorithms.AES(key), modes.CBC(metadata["iv"]), backend=default_backend()
            )
            ...
            # Decrypt
            cipher = Cipher(
                algorithms.ChaCha20(key, metadata["iv"]), mode=None, backend=default_backend()
            )
            decryptor = cipher.decryptor()
    ...
    def encrypt(
        ...
        master_key = self._derive_key(password, master_salt, 32, iterations=100000)  # 256-bit key
        ...
        for chunk_idx, chunk in enumerate(chunks):
            ...
            for layer_idx in range(num_layers):
                ...
                encrypted_data, layer_meta = self._encrypt_chunk(
                    current_data, layer_key, layer_config
                )
                ...
                layer_meta.update(
                    {
                        ...
                        "hmac_algorithm": (
                            layer_config.hmac_algorithm if layer_config.use_hmac else None
                        ),
    ...
    def decrypt(self, encrypted_data: bytes, metadata: Dict, password: bytes) -> bytes:
        """
            ...
            for layer_meta in reversed(layers_metadata):
                try:
                    ...
                    layer_key = self._derive_key(
                        master_key,
                        layer_salt
                        + struct.pack(">I", layer_meta_bytes["layer_idx"])
                    ...
                    current_data = self._decrypt_chunk(
                        current_data, layer_key, layer_config, layer_meta_bytes
                    )

                except Exception as e:
                    if (
                        self.enable_fail_safe and len(decrypted_chunks) == 0
