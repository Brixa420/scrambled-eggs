...
class ScrambledEggsCrypto:
    ...
    def encrypt(
        self, data: bytes, key: bytes = None, layer: EncryptionLayer = None
    ) -> EncryptionResult:
        """
    ...
    def _encrypt_aes_gcm(self, data: bytes, key: bytes, salt: bytes, iv: bytes) -> EncryptionResult:
        ...
        return EncryptionResult(
            ciphertext=ciphertext, key=key, salt=salt, iv=iv, auth_tag=encryptor.tag
        )
    ...
    def _encrypt_chacha20_poly1305(
        self, data: bytes, key: bytes, salt: bytes, iv: bytes
    ) -> EncryptionResult:
        ...
        # Create a cipher and encrypt the data
        encryptor = Cipher(
            algorithms.ChaCha20(derived_key, iv), mode=None, backend=self.backend
        ).encryptor()

        ciphertext = encryptor.update(data)

        # Generate a Poly1305 tag for authentication
        h = hmac_lib.HMAC(derived_key, hashes.SHA256(), backend=self.backend)
        h.update(iv + ciphertext)
        auth_tag = h.finalize()

        return EncryptionResult(ciphertext=ciphertext, key=key, salt=salt, iv=iv, auth_tag=auth_tag)
    ...
    def _decrypt_chacha20_poly1305(self, encrypted_data: EncryptionResult, key: bytes) -> bytes:
        ...
        # Decrypt the data
        decryptor = Cipher(
            algorithms.ChaCha20(derived_key, encrypted_data.iv), mode=None, backend=self.backend
        ).decryptor()

        return decryptor.update(encrypted_data.ciphertext)

    def _encrypt_scrambled_eggs(
        self, data: bytes, key: bytes, salt: bytes, iv: bytes
    ) -> EncryptionResult:
        ...
        chacha_result = self._encrypt_chacha20_poly1305(aes_result.ciphertext, key, salt, iv2)
    ...
    def _decrypt_scrambled_eggs(self, encrypted_data: EncryptionResult, key: bytes) -> bytes:
        ...
        aes_result = EncryptionResult(
            ciphertext=aes_ciphertext, key=key, salt=encrypted_data.salt, iv=iv1, auth_tag=tag1
        )
    ...
    def _generate_hmac(self, data: bytes, key: bytes, salt: bytes) -> bytes:
        ...
        h = hmac_lib.HMAC(self.derive_key(key, salt), hashes.SHA512(), backend=self.backend)
        h.update(data)
        return h.finalize()

    def reencrypt(
        self, encrypted_data: EncryptionResult, new_key: bytes = None
    ) -> EncryptionResult:
        ...
        return self.encrypt(
            decrypted,
            new_key,
            layer=encrypted_data.layer if hasattr(encrypted_data, "layer") else None,
        )
