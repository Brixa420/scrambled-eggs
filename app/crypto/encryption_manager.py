...
class KeyPair:
    ...
    def generate_rsa(cls, key_size: int = 2048) -> "KeyPair":
        ...
        return cls(public_key=key.publickey().export_key(), private_key=key.export_key())
    ...
    def from_private_key(cls, private_key: bytes, key_type: str = "rsa") -> "KeyPair":
        ...
        if key_type == "rsa":
            ...
            return cls(public_key=key.publickey().export_key(), private_key=private_key)
        elif key_type == "ecc":
            key = ECC.import_key(private_key)
            return cls(
                public_key=key.public_key().export_key(format="PEM"), private_key=private_key
            )
        else:
...
class EncryptionManager:
    ...
    def add_trusted_key(
        self, peer_id: str, public_key: bytes, signature_public_key: Optional[bytes] = None
    ) -> None:
    ...
    def encrypt_file(
        self, input_path: str, output_path: str, key: Optional[bytes] = None
    ) -> Tuple[bytes, bytes]:
