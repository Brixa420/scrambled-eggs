...
class ConnectionManager:
    ...
    def disconnect(self, client_id: ClientID):
        ...
        for offer_id in list(self.pending_offers.keys()):
            if (
                self.pending_offers[offer_id]["from"] == client_id
                or self.pending_offers[offer_id]["to"] == client_id
            ):
                del self.pending_offers[offer_id]

        for answer_id in list(self.pending_answers.keys()):
            if (
                self.pending_answers[answer_id]["from"] == client_id
                or self.pending_answers[answer_id]["to"] == client_id
            ):
        ...
        for peer_id in list(self.ice_candidates.keys()):
            if peer_id == client_id:
                ...
                self.ice_candidates[peer_id] = [
                    c
                    for c in self.ice_candidates[peer_id]
    ...
    async def get_peers(self, exclude_client_id: Optional[ClientID] = None) -> List["PeerInfo"]:
        ...
        return [
            peer_info
            for client_id, peer_info in self.peer_registry.items()
    ...
    async def handle_offer(self, from_client: ClientID, to_client: ClientID, offer: Dict):
        ...
        await self.send_message(
            to_client, {"type": "offer", "from": from_client, "offer": offer, "offer_id": offer_id}
        )

        return True

    async def handle_answer(
        self, from_client: ClientID, to_client: ClientID, answer: Dict, offer_id: str
    ):
    ...
    async def handle_ice_candidate(
        self, from_client: ClientID, to_client: ClientID, candidate: Dict
    ):
        ...
        self.ice_candidates[to_client].append(
            {"from": from_client, "candidate": candidate, "timestamp": time.time()}
        )

        # Forward the ICE candidate to the target peer
        await self.send_message(
            to_client, {"type": "ice-candidate", "from": from_client, "candidate": candidate}
        )
...
class PeerInfo(BaseModel):
    ...
    status: ContactStatus = Field(
        default=ContactStatus.ONLINE, description="Current status of the peer"
    )
...
class SignalingServer:
    ...
    def __init__(self, host: str = "0.0.0.0", port: int = 8765):
        ...
        async def get_public_key():
            return {"public_key": self.manager.server_key_pair.get_public_key().decode("utf-8")}
    ...
    async def handle_message(self, client_id: str, message: Dict):
        ...
        if msg_type == "register":
            ...
            await self.manager.broadcast(
                {"type": "peer-connected", "peer": peer_info.dict(exclude={"ip_address", "port"})},
                exclude=client_id,
            )
            ...
            if client_id in self.manager.peer_registry:
                ...
                await self.manager.broadcast(
                    {"type": "peer-updated", "client_id": client_id, "status": message["status"]},
                    exclude=client_id,
                )
