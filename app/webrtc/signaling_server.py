    ...
    def is_allowed(self, client_id: str) -> bool:
        ...
        if client_id in self.requests:
            self.requests[client_id] = [
                t for t in self.requests[client_id] if now - t < self.time_window
            ]
...
class WebRTCSignalingServer:
    ...
    def __init__(
        ...
        # Rate limiting
        self.rate_limiter = RateLimiter(max_requests=rate_limit, time_window=60)  # 1 minute
    ...
    async def handle_auth(self, request: web.Request) -> web.Response:
        ...
        return web.json_response(
            {"token": token, "peer_id": peer_id, "room_id": room_id, "expires_in": 300}  # 5 minutes
        )
    ...
    async def handle_message(self, peer_id: str, room_id: str, data: Dict[str, Any]) -> None:
        ...
        if msg_type in ["offer", "answer", "candidate"] and target_peer_id in self.sockets:
            await self.send_to_peer(
                target_peer_id, {**data, "sender_id": peer_id, "timestamp": time.time()}
            )
        else:
        ...
        if room_id in self.rooms and peer_id in self.rooms[room_id]:
            ...
            if room_id in self.rooms:  # Check if room still exists
                await self.notify_peers(
                    peer_id,
                    room_id,
                    {"type": "peer-left", "peer_id": peer_id, "timestamp": time.time()},
                )
            ...
            try:
                ...
                if peer_id in self.sockets:
                    await self.handle_disconnect(
                        peer_id, self.peer_info[peer_id].get("room_id", "unknown")
                    )
