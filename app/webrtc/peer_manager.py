...
class WebRTCPeerManager:
    ...
    def __init__(
        ...
        self.signaling_url = f"{signaling_url}?peer_id={peer_id}&room_id={room_id}"
    ...
    async def _create_peer_connection(self, peer_id: str) -> RTCPeerConnection:
        ...
        if peer_id > self.peer_id:  # Simple way to decide who initiates
            ...
            await self._send_signaling_message(peer_id, {
                'type': 'offer',
                'sdp': object_to_string(pc.localDescription)
            })
        ...
        def on_message(message):
            try:
                if isinstance(message, str):
                    ...
                    logger.warning(f"Received binary message from {peer_id}, which is not supported")
