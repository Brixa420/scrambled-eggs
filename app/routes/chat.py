        ...
        [
            {
                ...
                "user": {"id": msg.user_id, "username": msg.author.username},
            }
            for msg in messages
        ]
    )
...
def send_message():
    ...
    message = Message(content=data["content"], user_id=current_user.id, timestamp=datetime.utcnow())
...
def handle_connect():
    ...
    if current_user.is_authenticated:
        ...
        socketio.emit(
            "user_connected", {"user_id": current_user.id, "username": current_user.username}
        )
...
def handle_disconnect():
    ...
    if current_user.is_authenticated:
        ...
        socketio.emit(
            "user_disconnected", {"user_id": current_user.id, "username": current_user.username}
        )
