...
class MetricsStorage:
    ...
    def get_metrics_summary(
        ...
        with self.Session() as session:
            ...
            return {
                ...
                "avg_latency": (
                    float(circuit_metrics.avg_latency)
                    if circuit_metrics.avg_latency is not None
                    else None
                ),
                "request_rate": total_requests / time_span_seconds if time_span_seconds > 0 else 0,
                "error_rate": (total_errors / total_requests * 100) if total_requests > 0 else 0,
                "throughput_up": (
                    total_bytes_sent / time_span_seconds if time_span_seconds > 0 else 0
                ),
                "throughput_down": (
                    total_bytes_received / time_span_seconds if time_span_seconds > 0 else 0
                ),
    ...
    def cleanup_old_metrics(self, max_age_days: int = 30) -> int:
        ...
        with self.Session() as session:
            ...
            circuit_count = (
                session.query(CircuitMetricsModel)
            ...
            aggregate_count = (
                session.query(MetricsAggregate)
