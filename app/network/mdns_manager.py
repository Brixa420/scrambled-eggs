...
class MDNSListener(ServiceListener):
    ...
    def __init__(
        self,
        on_peer_added: Callable[[str, PeerInfo], None] = None,
    ...
    def add_service(self, zc: Zeroconf, type_: str, name: str) -> None:
        ...
        # Create peer info
        peer = PeerInfo(
            name=name, address=address, port=info.port, properties=info.properties or {}
        )
...
class MDNSManager:
    ...
    def __init__(
        self,
        service_name: str = "_scrambledeggs._tcp.local.",
        service_port: int = 0,
    ...
    def start(
        ...
        self.thread = threading.Thread(
            target=self._run, args=(on_peer_added, on_peer_removed), daemon=True
        )
        self.thread.start()
        self.running = True
    ...
    def _run(self, on_peer_added, on_peer_removed) -> None:
        ...
        try:
            ...
            self.browser = ServiceBrowser(self.zeroconf, self.service_name, listener=self.listener)
    ...
    def _advertise_service(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"mDNS service registered: {self.service_info.name} at {local_ip}:{self.service_port}"
            )
