...
from typing import Dict, List, Optional, Tuple, BinaryIO, Callable, Any
...
class FileSharingManager:
    ...
    async def share_file(self, file_path: str, peer_id: str, chunk_size: int = DEFAULT_CHUNK_SIZE) -> str:
        ...
        if file_size > MAX_FILE_SIZE:
            raise FileTooLargeError(f"File too large: {file_size} bytes (max {MAX_FILE_SIZE} bytes)")
        
        # Generate file metadata
        file_id = hashlib.sha256(
            f"{file_path.name}{file_size}{time.time()}".encode()
        ).hexdigest()
        ...
        await self.p2p_manager.send_message(peer_id, {
            'type': 'file_offer',
            'file_id': file_id,
        ...
        try:
            ...
            if 'checksum' in message:
                ...
                if file_checksum != message['checksum']:
                    logger.error(f"File checksum mismatch: {file_checksum} != {message['checksum']}")
        ...
        try:
            with open(upload['file_path'], 'rb') as f:
                while upload['chunks_sent'] < metadata.chunks:
                    ...
                    # Send chunk
                    await self.p2p_manager.send_message(upload['peer_id'], {
                        'type': 'file_chunk',
                        'file_id': file_id,
    ...
    def get_download_progress(self, file_id: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            'progress': (download['chunks_received'] / metadata.chunks * 100) if hasattr(metadata, 'chunks') and metadata.chunks > 0 else 0,
