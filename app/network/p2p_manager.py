...
from ..security.gate_system import GateSystem
...
class P2PManager:
    ...
    async def _cleanup_expired_entries(self) -> None:
        ...
        expired_peers = [
            peer_id for peer_id, (expiry, _) in self.blacklist.items() if expiry < current_time
        ]
        ...
        expired_sessions = [
            peer_id
            for peer_id, key_info in self.session_keys.items()
    ...
    async def _check_security_status(self) -> None:
        ...
        for peer_id, peer_info in list(self.peers.items()):
            if (
                peer_info.error_count > 10
                and peer_info.last_error
                and (current_time - peer_info.last_error) < 3600
            ):
                # Too many recent errors, blacklist the peer
                await self._blacklist_peer(
                    peer_id, reason=f"Too many errors: {peer_info.error_count} in the last hour"
                )
    ...
    async def _enforce_rate_limit(self, peer_info: PeerInfo) -> bool:
        ...
        if peer_info.message_rate > self.config["max_message_rate"]:
            logger.warning(
                f"Rate limit exceeded for {peer_info.peer_id}: {peer_info.message_rate:.1f} msgs/sec"
            )
        ...
        try:
            ...
            async with self.peer_lock:
                if peer_id in self.peers or len(self.peers) >= self.max_peers:
                    logger.warning(
                        f"Rejecting connection from {peer_id}: {'Peer already connected' if peer_id in self.peers else 'Max peers reached'}"
                    )
                    writer.close()
                    await writer.wait_closed()
                    return
            ...
            logger.info(
                f"Established connection with peer {peer_id} at {peer_addr[0]}:{peer_addr[1]}"
            )
    ...
    async def _handle_peer_messages(
        self, reader: asyncio.StreamReader, writer: asyncio.StreamWriter, peer_info: PeerInfo
    ) -> None:
        ...
        try:
                ...
                try:
                    ...
                    if await self._enforce_rate_limit(peer_info):
                        ...
                        await self._send_error(
                            peer_info, "rate_limit_exceeded", "Rate limit exceeded"
                        )
                        break
                    ...
                    logger.error(
                        f"Unexpected error processing message from {peer_id}: {e}", exc_info=True
                    )
                    peer_info.record_error()
            ...
            logger.error(f"Fatal error in message handler for {peer_id}: {e}", exc_info=True)
        finally:
            # Clean up resources
    ...
    async def _process_message(self, data: bytes, peer_info: PeerInfo) -> None:
        ...
        try:
            ...
            try:
                ...
                self.logger.debug(
                    f"Received message from {peer_info.peer_id}: {message.get('type')}"
                )
            ...
            if message.get("type") == "encrypted_message":
                ...
                if not decrypted:
                    ...
                    await self._send_error(
                        peer_info, "decryption_error", "Failed to decrypt message"
                    )
                    return
                message = decrypted
            ...
            if message_id and message_id in peer_info.received_message_ids:
                self.logger.debug(
                    f"Ignoring duplicate message {message_id} from {peer_info.peer_id}"
                )
                return
            ...
            if message_id:
                ...
                if len(peer_info.received_message_ids) > 1000:
                    peer_info.received_message_ids = set(
                        list(peer_info.received_message_ids)[-1000:]
                    )
            ...
            if not handler:
                self.logger.debug(
                    f"No handler for message type '{message_type}' from {peer_info.peer_id}"
                )
                await self._send_error(
                    peer_info, "unknown_message_type", f"Unknown message type: {message_type}"
                )
                return
            ...
            try:
                ...
                if not await self.gate_system.check_message_allowed(peer_info, message_type):
                    self.logger.warning(
                        f"Message type '{message_type}' not allowed from {peer_info.peer_id}"
                    )
                    await self._send_error(
                        peer_info,
                        "message_not_allowed",
                ...
                self.logger.error(
                    f"Error processing {message_type} from {peer_info.peer_id}: {e}", exc_info=True
                )
                await self._send_error(
                    peer_info, "processing_error", f"Error processing {message_type}: {str(e)}"
                )

        except Exception as e:
            self.logger.error(
                f"Unexpected error in _process_message for {peer_info.peer_id}: {e}", exc_info=True
            )
    ...
    async def _cleanup_peer_connection(
        self, peer_info: PeerInfo, writer: asyncio.StreamWriter
    ) -> None:
    ...
    async def _broadcast_peers(self, exclude_peer_id: Optional[str] = None) -> None:
        ...
        message = {"type": "peers_list", "peers": peers_list, "timestamp": time.time()}

        await self.broadcast_message(
            message, exclude={exclude_peer_id} if exclude_peer_id else None
        )
        ...
        try:
            ...
            if "signature" in message and peer_info.public_key:
                if not self._verify_signature(
                    message["public_key"], self._get_signing_payload(message), message["signature"]
                ):
            ...
            peer_info.public_key = serialization.load_pem_public_key(
                message["public_key"].encode("utf-8"), backend=default_backend()
            )
    ...
    async def _handle_handshake_response(
        self, message: Dict[str, Any], peer_info: PeerInfo
    ) -> None:
        ...
        try:
            ...
            if "signature" in message:
                if not self._verify_signature(
                    ...
                    logger.warning(
                        f"Invalid signature in handshake response from {peer_info.peer_id}"
                    )
                    return
            ...
            logger.error(
                f"Error processing handshake response from {peer_info.peer_id}: {e}", exc_info=True
            )
            peer_info.record_error()
    ...
    async def _request_peers(self, peer_info: PeerInfo) -> None:
        ...
        """
        try:
            message = {"type": "get_peers", "timestamp": time.time()}
    ...
    async def _handle_get_peers(self, message: Dict[str, Any], peer_info: PeerInfo) -> None:
        ...
        try:
            ...
            if peers_list:
                response = {"type": "peers_list", "peers": peers_list, "timestamp": time.time()}
    ...
    async def _handle_peers_list(self, message: Dict[str, Any], peer_info: PeerInfo) -> None:
        ...
        try:
            ...
            logger.error(
                f"Error processing peers list from {peer_info.peer_id}: {e}", exc_info=True
            )
            peer_info.record_error()
    ...
    async def _handle_error(self, message: Dict[str, Any], peer_info: PeerInfo) -> None:
        ...
        try:
            ...
            logger.error(
                f"Error processing error message from {peer_info.peer_id}: {e}", exc_info=True
            )
            peer_info.record_error()
    ...
    async def _handle_ping(self, message: Dict[str, Any], peer_info: PeerInfo) -> None:
        ...
        try:
            ...
            # Send pong response
            pong = {"type": "pong", "timestamp": message.get("timestamp"), "node_time": time.time()}
    ...
    async def _handle_gate_sequence(self, message: Dict[str, Any], peer_info: PeerInfo) -> None:
        ...
        try:
            ...
            logger.debug(
                f"Updated gate sequence for peer {peer_info.peer_id}: {peer_info.gate_sequence}"
            )
            ...
            logger.error(
                f"Error handling gate sequence from {peer_info.peer_id}: {e}", exc_info=True
            )
            peer_info.record_error()
    ...
    async def _send_message(self, peer_info: PeerInfo, message: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if len(message_bytes) > self.config["max_message_size"]:
                logger.warning(
                    f"Message too large ({len(message_bytes)} bytes) for {peer_info.peer_id}"
                )
                return False
    ...
    def _sign_message(self, message: Dict[str, Any]) -> str:
        ...
        try:
            ...
            # Sign the message
            signature = self.private_key.sign(msg_str.encode("utf-8"), ec.ECDSA(hashes.SHA256()))
