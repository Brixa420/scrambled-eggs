...
class TorManager:
    ...
    def _find_tor_binary(self) -> Optional[str]:
        ...
        try:
            tor_path = subprocess.check_output(["which", "tor"], stderr=subprocess.PIPE).decode().strip()
            if tor_path:
                return tor_path
    ...
    def start_tor(self) -> bool:
        ...
        try:
            ...
            torrc_config = {
                ...
                'Log': [
                    'NOTICE file ' + str(self.tor_log),
                    'ERR file ' + str(self.tor_log)
                ],
            ...
            for service_name, service_config in self.hidden_services.items():
                ...
                torrc_config[f'HiddenServicePort'] = f"{service_config['virtual_port']} {service_config['target']}"
...
if __name__ == "__main__":
    ...
    tor = TorManager(
        tor_data_dir=os.path.join(temp_dir, "tor_data"),
        tor_log=os.path.join(temp_dir, "tor.log")
    )
