...
class TorManager:
    ...
    def start(self) -> bool:
        ...
        try:
            ...
            logger.info(f"Tor manager started successfully (control_port={self.control_port}, socks_port={self.socks_port})")
            return True
            ...
            try:
                if self.password:
                    ...
                    try:
                        ...
                        logger.warning(f"Cookie authentication failed: {e}. Trying without authentication...")
    ...
    def _start_tor(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                logger.info(f"Using control port: {self.control_port}, SOCKS port: {self.socks_port}")
            except Exception as e:
            ...
            if not self.tor_binary:
                ...
                if not self.tor_binary:
                    logger.error("Could not find Tor binary. Please install Tor Browser or specify the path.")
                    return False
    ...
    def _handle_tor_event(self, event: stem.response.events.Event) -> None:
        ...
        try:
            ...
            elif isinstance(event, (stem.response.events.NoticeEvent, 
                                 stem.response.events.WarnEvent,
                                 stem.response.events.ErrorEvent)):
                logger.log(
                    logging.WARNING if isinstance(event, stem.response.events.WarnEvent) else 
                    logging.ERROR if isinstance(event, stem.response.events.ErrorEvent) else
                    logging.INFO,
                    f"Tor: {event}"
                )
        except Exception as e:
    ...
    def _handle_circuit_event(self, event: stem.response.events.CircuitEvent) -> None:
        ...
        with self.circuit_lock:
            ...
            if event.status == 'BUILT':
                ...
                if circuit_id in self.circuits:
                    circuit = self.circuits[circuit_id]
                    circuit.state = TorCircuitState.FAILED if event.status == 'FAILED' else TorCircuitState.CLOSED
                    logger.warning(f"Circuit {circuit_id} {event.status.lower()}: purpose={circuit.purpose}")
        ...
        try:
            ...
            with self.circuit_lock:
                ...
                for circuit_id in circuits_to_close:
                    if circuit_id in self.circuits:
                        circuit = self.circuits[circuit_id]
                        logger.debug(f"Closing circuit {circuit_id} (purpose={circuit.purpose}, "
        ...
        with self.circuit_lock:
            ...
            try:
                ...
                self.controller.new_circuit(
                    await_build=True,
                    purpose=purpose,
                    isolation_flags=['IsolateDestPort', 'IsolateDestAddr'] if self.purpose_isolation.get(purpose, True) else []
                )
                ...
                while (circuit.state != TorCircuitState.READY and 
    ...
    def get_connection_status(self) -> dict:
        ...
        try:
            ...
            return {
                ...
                'entry_guards': tor_info.get('entry-guards', '').split('\n') if 'entry-guards' in tor_info else []
            }
