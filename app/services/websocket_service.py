...
from app.models.file import File, FileAccessLog, FileShare
...
class WebSocketService:
    ...
    def _get_user_from_token(self, token: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            return {"id": payload["identity"], "username": payload.get("username", "anonymous")}
    ...
    def _handle_join_file_room(self, data: Dict[str, Any]):
        ...
        self.socketio.emit(
            "user_joined",
            {"user_id": user_id, "file_id": file_id, "timestamp": datetime.utcnow().isoformat()},
            room=file_id,
            include_self=False,
        )
    ...
    def _handle_file_update(self, data: Dict[str, Any]):
        ...
        if file.user_id != user_id:
            ...
            share = FileShare.query.filter_by(
                file_id=file_id, shared_with=user_id, can_edit=True
            ).first()
