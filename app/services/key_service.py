...
from app.services.encryption import ScrambledEggsCrypto, crypto
...
class KeyService:
    ...
    def _enforce_key_history_limit(self) -> None:
        ...
        for key_id, key_data in self.keys_metadata["key_versions"].items():
            if key_data["is_active"] or datetime.fromisoformat(key_data["last_used"]) > datetime.utcnow() - timedelta(days=30):
        ...
        for key_id in keys_to_remove:
            ...
            self.keys_metadata["key_history"] = [
                entry for entry in self.keys_metadata["key_history"]
    ...
    def get_key_rotation_schedule(self) -> dict:
        ...
        return {
            ...
            "active_keys": sum(1 for k in self.keys_metadata["key_versions"].values() if k["is_active"])
        }
