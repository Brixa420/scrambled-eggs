...
async def download_file(file_id):
    ...
    try:
        ...
        if not file.is_public and current_user_id != file.user_id:
            ...
            access = FileAccess.query.filter_by(file_id=file_id, user_id=current_user_id).first()
        ...
        try:
            decrypted_data = file_service.download_file(
                file_id=file_id, password=password, key=file.encryption_key
            )
...
async def share_file(file_id):
    ...
    try:
        ...
        return jsonify({"message": "File shared successfully", "share_id": share.id}), 201
