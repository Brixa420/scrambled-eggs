...
from app.core.contact_manager import ContactManager
...
def handle_send_message(data: Dict[str, Any]) -> None:
    ...
    try:
        ...
        if not all([recipient_id, encrypted_content, message_id]):
            emit("message_error", {"message_id": message_id, "error": "Missing required fields"})
            return
        ...
        emit("message_error", {"message_id": data.get("message_id", "unknown"), "error": str(e)})
...
def handle_start_file_transfer(data: Dict[str, Any]) -> None:
    ...
    try:
        ...
        if not all([recipient_id, file_metadata]):
            emit(
                "file_transfer_error",
                {"transfer_id": data.get("transfer_id"), "error": "Missing required fields"},
            )
            return
        ...
        emit(
            "file_transfer_error",
            {"transfer_id": data.get("transfer_id", "unknown"), "error": str(e)},
        )
...
def health_check() -> Response:
    ...
    return jsonify(
        {"status": "ok", "timestamp": datetime.now(timezone.utc).isoformat(), "version": "1.0.0"}
    )
...
def get_contacts() -> Response:
    ...
    try:
        ...
        return jsonify(
            {"status": "success", "contacts": [contact.to_dict() for contact in contacts]}
        )
    except Exception as e:
