...
class EncryptionMonitor:
    ...
    def _init_metrics(self):
        ...
        self.key_age_days = Gauge(
            "scrambled_eggs_encryption_key_age_days", "Age of encryption keys in days", ["key_type"]
        )
    ...
    def _record_metrics(self, metrics: OperationMetrics):
        ...
        self.ops_total.labels(operation=metrics.operation.value, status=status).inc()

        # Record operation duration
        if metrics.duration_seconds is not None:
            self.ops_duration.labels(operation=metrics.operation.value).observe(
                metrics.duration_seconds
            )

        # Record data size if available
        if metrics.data_size is not None:
            self.data_size_bytes.labels(operation=metrics.operation.value).inc(metrics.data_size)
    ...
    def record_key_generated(self, key_id: str, key_type: str, key_size: int):
        ...
        logger.info(
            ...
            extra={"key_id": key_id, "key_type": key_type, "key_size": key_size},
        )
    ...
    def record_key_rotated(self, old_key_id: str, new_key_id: str, key_type: str):
        ...
        logger.info(
            ...
            extra={"old_key_id": old_key_id, "new_key_id": new_key_id, "key_type": key_type},
        )
    ...
    def record_key_expired(self, key_id: str, key_type: str):
        ...
        logger.info("Encryption key expired", extra={"key_id": key_id, "key_type": key_type})
