...
try:
    from app.core.crypto import CryptoEngine, EncryptionResult as OldEncryptionResult
    from app.crypto.scrambled_eggs import ScrambledEggsCrypto, EncryptionResult
            ...
            with get_db() as db:
                ...
                for key in keys:
                    try:
                        ...
                        migrator.metrics['migration_errors'].append(f"Failed to migrate key {getattr(key, 'id', 'unknown')}: {str(e)}")
        ...
        try:
            ...
            for data in encrypted_data:
                try:
                    ...
                    migrator.metrics['failed_migrations'] += 1
                    migrator.metrics['migration_errors'].append(f"Failed to migrate data {data.get('id', 'unknown')}: {str(e)}")
