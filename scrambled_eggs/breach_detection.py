...
class BreachDetector:
    ...
    def _detect_unusual_process_activity(self) -> List[BreachAlert]:
        ...
        alerts = []
        current_processes = {p.pid: p for p in psutil.process_iter(['pid', 'name', 'username', 'cmdline'])}
        ...
        for proc in current_processes.values():
            try:
                proc_info = proc.info
                cmd_line = ' '.join(proc_info.get('cmdline', [])) if proc_info.get('cmdline') else ''
                
                for keyword in suspicious_keywords:
                    if keyword in cmd_line.lower():
                        alerts.append(BreachAlert(
    ...
    def analyze_system_health(self) -> Dict:
        ...
        if health['metrics']['memory_usage'] > 90:
            health['security_score'] -= 10
            health['recommendations'].append("High memory usage detected. Check for memory leaks or malware.")
