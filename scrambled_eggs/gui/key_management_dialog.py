...
from PySide6.QtWidgets import (
    ...
    QMessageBox, QTabWidget, QWidget, QFormLayout, QGroupBox,
    QComboBox, QCheckBox, QDateEdit, QDateTimeEdit, QTextEdit,
    QFileDialog, QMenu, QInputDialog, QSpinBox, QTreeWidget, 
    QTreeWidgetItem, QAbstractItemView, QSplitter, QToolBar, 
    QStatusBar, QSizePolicy, QDialogButtonBox
)
...
class KeyManagementDialog(QDialog):
    ...
    def filter_keys(self, text):
        ...
        for i in range(self.key_list.topLevelItemCount()):
            ...
            for j in range(owner_item.childCount()):
                ...
                if (search_text in key_item.text(0).lower() or
                    (key_data and search_text in key_data.get("id", "").lower()) or
                    (key_data and search_text in key_data.get("email", "").lower())):
    ...
    def show_key_details(self, key_data):
        ...
        self.key_fingerprint_edit.setText(":".join(
            key_data.get("id", "")[i:i+2] for i in range(0, min(16, len(key_data.get("id", ""))), 2)
        ))
    ...
    def import_key(self):
        ...
        file_path, _ = QFileDialog.getOpenFileName(
            ...
            "Import Key",
            "",
            "Key Files (*.key *.pem *.der);;All Files (*)"
        )
