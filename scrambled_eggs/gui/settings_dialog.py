...
from PySide6.QtWidgets import (
    ...
    QComboBox, QCheckBox, QSpinBox, QDoubleSpinBox, QFileDialog,
    QMessageBox, QListWidget, QListWidgetItem, QTreeWidget, QTreeWidgetItem,
    QHeaderView, QTableWidget, QTableWidgetItem, QAbstractItemView,
    QDialogButtonBox, QStackedWidget, QScrollArea, QFrame, QSplitter
)
from PySide6.QtCore import Qt, QSize, QSettings, QStandardPaths
from PySide6.QtGui import QIcon, QPixmap, QColor, QFont, QIntValidator, QDoubleValidator
...
class SettingsDialog(QDialog):
    ...
    def create_security_page(self):
        ...
        self.strength_combo.addItems(["Low (faster)", "Medium", "High (slower)", "Paranoid (slowest)"])
    ...
    def update_encryption_ui(self):
        ...
        if enabled:
            ...
            self.encryption_status.setText("⚠️ Encryption is disabled - messages will be sent in plain text")
    ...
    def create_appearance_page(self):
        ...
        self.toolbar_combo.addItems(["Icons Only", "Text Only", "Text Beside Icons", "Text Under Icons"])
    ...
    def create_shortcuts_page(self):
        ...
        self.shortcuts_table.horizontalHeader().setSectionResizeMode(2, QHeaderView.ResizeMode.Stretch)
    ...
    def copy_system_info(self):
        ...
        import platform
        import sys
        from PySide6.QtCore import QSysInfo, QLibraryInfo
    ...
    def browse_save_location(self):
        ...
        dir_path = QFileDialog.getExistingDirectory(
            ...
            self.save_location_edit.text() or str(Path.home())
        )
    ...
    def load_settings(self):
        ...
        self.theme_combo.setCurrentText(
            self.settings.value("general/theme", "System")
        )
        self.updates_check.setChecked(
            self.settings.value("general/check_updates", True, type=bool)
        )
        self.beta_check.setChecked(
            self.settings.value("general/beta_updates", False, type=bool)
        )
        
        # File handling
        default_save = str(Path.home() / "ScrambledEggs")
        self.save_location_edit.setText(
            self.settings.value("file/save_location", default_save)
        )
        self.file_history_spin.setValue(
            self.settings.value("file/history_size", 10, type=int)
        )
        self.autosave_check.setChecked(
            self.settings.value("file/autosave", True, type=bool)
        )
        self.autosave_spin.setValue(
            self.settings.value("file/autosave_interval", 5, type=int)
        )
        
        # Security settings
        self.autolock_check.setChecked(
            self.settings.value("security/autolock", True, type=bool)
        )
        self.autolock_spin.setValue(
            self.settings.value("security/autolock_timeout", 15, type=int)
        )
        ...
        # Privacy settings
        self.analytics_check.setChecked(
            self.settings.value("privacy/analytics", True, type=bool)
        )
        self.crash_check.setChecked(
            self.settings.value("privacy/crash_reports", True, type=bool)
        )
        ...
        self.kdf_combo.setCurrentText(
            self.settings.value("encryption/kdf", "PBKDF2-HMAC-SHA256")
        )
        self.iterations_spin.setValue(
            self.settings.value("encryption/iterations", 600000, type=int)
        )
        self.memory_spin.setValue(
            self.settings.value("encryption/memory", 64, type=int)
        )
        self.parallelism_spin.setValue(
            self.settings.value("encryption/parallelism", 4, type=int)
        )
        
        # Network settings
        self.proxy_combo.setCurrentText(
            self.settings.value("network/proxy_type", "No Proxy")
        )
        self.proxy_host_edit.setText(
            self.settings.value("network/proxy_host", "")
        )
        self.proxy_port_edit.setText(
            self.settings.value("network/proxy_port", "")
        )
        self.proxy_auth_check.setChecked(
            self.settings.value("network/proxy_auth", False, type=bool)
        )
        self.proxy_user_edit.setText(
            self.settings.value("network/proxy_username", "")
        )
        self.proxy_pass_edit.setText(
            self.settings.value("network/proxy_password", "")
        )
        self.timeout_spin.setValue(
            self.settings.value("network/timeout", 30, type=int)
        )
        self.retries_spin.setValue(
            self.settings.value("network/retries", 3, type=int)
        )
        
        # Appearance settings
        self.theme_combo.setCurrentText(
            self.settings.value("appearance/theme", "System")
        )
        self.font_size_spin.setValue(
            self.settings.value("appearance/font_size", 10, type=int)
        )
        self.icon_theme_combo.setCurrentText(
            self.settings.value("appearance/icon_theme", "System")
        )
