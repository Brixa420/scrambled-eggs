...
from PySide6.QtCore import QDateTime, QObject, QSettings, QSize, QSizeF, Qt, QTimer, QUrl, Signal
...
from ..database.models import Contact, Database, Message
...
class ChatMessageWidget(QWidget):
    ...
    def __init__(
        self, sender: str, message: str, timestamp: str, is_self: bool = False, parent=None
    ):
        super().__init__(parent)
        self.setLayout(QVBoxLayout())
...
class ChatWindow(QWidget):
    ...
    def load_contacts(self):
        ...
        sample_contact = Contact(
            id="sample_user", name="Sample User", public_key="sample_public_key", online=True
        )
    ...
    def filter_contacts(self):
        ...
        for i in range(self.contacts_list.count()):
            ...
            if contact and (
                search_text in contact.name.lower() or search_text in contact.id.lower()
            ):
    ...
    def load_chat_history(self):
        ...
        self.add_message(
            "Sample User", "Hello! This is a sample message.", "12:34 PM", is_self=False
        )
    ...
    def add_message(self, sender: str, message: str, timestamp: str, is_self: bool = False):
        ...
        self.message_document.addResource(
            QTextDocument.ResourceType.ImageResource, QUrl("widget"), QVariant()
        )
    ...
    def update_encryption_status(self):
        ...
        if self.encryption_enabled:
            ...
            self.encryption_status.setToolTip(
                "End-to-end encryption is disabled - messages will be sent in plain text"
            )
    ...
    def send_message(self):
        ...
        try:
            ...
            encrypted_data = self.hybrid_encryption.encrypt(
                message_text, recipient_public_key=self.current_contact.public_key
            )

            # Convert to JSON string for sending
            message_to_send = json.dumps({"encrypted": True, "data": encrypted_data})

            # Add to chat display (only after successful encryption)
            self.add_message("You", message_text, timestamp, is_self=True)
            self.add_message(
                "System", "🔒 Message sent with end-to-end encryption", timestamp, is_self=True
            )
            ...
            # Send via WebRTC
            asyncio.create_task(
                self.webrtc_manager.send_message(self.current_contact.id, message_to_send)
            )
            ...
            try:
                ...
                if isinstance(message_data, dict) and message_data.get("encrypted", False):
                    ...
                    if self.current_contact and self.current_contact.id == sender_id:
                        ...
                        self.add_message(
                            "System",
                            "🔒 Message received with end-to-end encryption",
                            timestamp,
                            is_self=False,
                        )
