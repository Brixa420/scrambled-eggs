...
async def list_contacts(args):
    ...
    try:
        ...
        for contact_id, contact in contacts.items():
            status = "[green]Online[/green]" if contact.get('online', False) else "[red]Offline[/red]"
            table.add_row(
                contact_id[:8] + "...",
...
def main(args: Optional[list] = None) -> int:
    ...
    encrypt_parser.add_argument('-l', '--layers', type=int, default=100,
                              help='Number of hashing layers (default: 100)')
    encrypt_parser.add_argument('-f', '--force', action='store_true',
    ...
    decrypt_parser.add_argument('-f', '--force', action='store_true',
                              help='Overwrite output file if it exists')
