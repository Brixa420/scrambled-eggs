...
class AISecurityAgent:
    ...
    def _initialize_threat_patterns(self) -> Dict[ThreatType, Dict]:
        ...
        return {
            ThreatType.BRUTE_FORCE: {
                "indicators": ["repeated_failures", "high_frequency_attempts", "similar_passwords"],
                "responses": ["add_encryption_layer", "temporarily_block_ip", "require_captcha"],
                "base_confidence": 0.7,
            },
            ThreatType.TIMING_ATTACK: {
                ...
                "responses": ["add_timing_noise", "add_encryption_layer", "require_reauth"],
                "base_confidence": 0.6,
            },
            ThreatType.REPLAY_ATTACK: {
                "indicators": ["duplicate_messages", "old_timestamps", "sequence_anomalies"],
                "responses": ["invalidate_session", "require_reauth", "rotate_encryption_keys"],
                "base_confidence": 0.8,
            },
        }
    ...
    def _calculate_confidence(self, threat_type: ThreatType, event_data: Dict[str, Any]) -> float:
        ...
        recent_similar = sum(
            1
            for t in self.threat_history[-10:]
            if t.threat_type == threat_type and time.time() - t.timestamp < 3600
        )  # Last hour
    ...
    def _generate_actions(
        self, threat_type: ThreatType, confidence: float, event_data: Dict[str, Any]
    ) -> List[str]:
    ...
    def learn_from_feedback(self, assessment: ThreatAssessment, was_correct: bool):
        ...
        if was_correct:
            ...
            if assessment.threat_type in self.threat_patterns:
                ...
                self.threat_patterns[assessment.threat_type]["base_confidence"] = min(
                    0.95,
                    self.threat_patterns[assessment.threat_type]["base_confidence"]
                    + self.learning_rate,
                )
        else:
            # Learn from mistake
        ...
        logger.info(
            f"AI learned from feedback. New confidence for {assessment.threat_type}: "
