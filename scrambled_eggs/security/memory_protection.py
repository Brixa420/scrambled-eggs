...
import threading
import weakref
from ctypes import POINTER, Structure, c_char_p, c_size_t, c_ubyte, c_void_p
        ...
        try:
            if platform.system() == "Windows":
                ...
                if platform.system() == "Linux":
                    ...
                    if HAS_MADVISE:
                        self._madvise(
                            ctypes.addressof(self._address), aligned_size, self.MADV_DONTDUMP
                        )

                    # Wipe memory on fork
                    if self._wipe_on_fork and HAS_MADVISE:
                        self._madvise(
                            ctypes.addressof(self._address), aligned_size, self.MADV_WIPEONFORK
                        )
    ...
    def protect(self, read_only: bool = True) -> None:
        ...
        try:
            if platform.system() == "Windows":
                ...
                if not self._virtual_protect(
                    self._address, self._size, protect, ctypes.byref(old_protect)
                ):
...
def secure_hash(
    data: Union[str, bytes], algorithm: str = "sha256", salt: Optional[bytes] = None
) -> bytes:
