...
class KeyDerivation:
    ...
    def _scrypt_derive(self, password: bytes, key_length: int) -> bytes:
        ...
        # Derive the key
        dk = hashlib.scrypt(password=password, salt=self.salt, n=n, r=r, p=p, dklen=key_length)
    ...
    def __eq__(self, other: object) -> bool:
        ...
        return (
            self.kdf_type == other.kdf_type
            and self.hash_alg == other.hash_alg
...
def derive_key(
    password: Union[str, bytes], salt: Optional[bytes] = None, key_length: int = 32, **kwargs
) -> bytes:
