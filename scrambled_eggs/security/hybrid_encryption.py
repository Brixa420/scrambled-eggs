...
class HybridEncryption:
    ...
    def encrypt(self, data: Union[bytes, str], recipient_public_key=None) -> Dict[str, Any]:
        ...
        try:
            ...
            encrypted_key = recipient_public_key.encrypt(symmetric_key, self.rsa_padding)
    ...
    def decrypt(self, encrypted_data: Dict[str, Any], private_key=None) -> bytes:
        ...
        try:
            ...
            symmetric_key = private_key.decrypt(encrypted_key, self.rsa_padding)

            # Create cipher and decrypt the data
            cipher = Cipher(
                algorithms.AES(symmetric_key), modes.GCM(iv, tag), backend=default_backend()
            )
