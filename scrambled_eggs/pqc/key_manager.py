...
from .hsm import HSMInterface, HSMKey, HSMType, KeyType, KeyUsage
...
class KeyMetadata:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        data = {
            ...
            "wrapped_key": (
                base64.b64encode(self.wrapped_key).decode("ascii") if self.wrapped_key else None
            ),
            "wrapped_key_algorithm": self.wrapped_key_algorithm,
            "wrapped_key_iv": (
                base64.b64encode(self.wrapped_key_iv).decode("ascii")
                if self.wrapped_key_iv
                else None
            ),
            "wrapped_key_tag": (
                base64.b64encode(self.wrapped_key_tag).decode("ascii")
                if self.wrapped_key_tag
                else None
            ),
        }
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "KeyMetadata":
        ...
        if "purpose" in known and isinstance(known["purpose"], list):
            known["purpose"] = [
                KeyPurpose[p] if isinstance(p, str) else p for p in known["purpose"]
            ]
...
class KeyManager:
    ...
    def _init_master_key(
        self, master_key: bytes = None, password: str = None, salt: bytes = None
    ) -> None:
    ...
    def _derive_key(
        ...
        dk = hashlib.pbkdf2_hmac("sha256", password, salt, iterations=iterations, dklen=key_size)
    ...
    def _unwrap_key(
        self, wrapped_key: bytes, iv: bytes, tag: bytes, wrapping_key_id: str = None
    ) -> bytes:
        """
            ...
            if key_id in self._metadata and self._metadata[key_id].is_wrapped:
                ...
                try:
                    key = self._unwrap_key(
                        metadata.wrapped_key, metadata.wrapped_key_iv, metadata.wrapped_key_tag
                    )
        ...
        if self.storage_path and (self.storage_path / f"{key_id}.key").exists():
            try:
                ...
                if key_id in self._metadata and self._metadata[key_id].is_wrapped:
                    ...
                    try:
                        key = self._unwrap_key(
                            key, metadata.wrapped_key_iv, metadata.wrapped_key_tag
                        )
                    ...
                    try:
                        if isinstance(pub_key, ec.EllipticCurvePublicKey):
                            ...
                            if algorithm and "pss" in algorithm.lower():
                                ...
                                # PKCS#1 v1.5
                                pub_key.verify(signature, data, padding.PKCS1v15(), hash_alg)
    ...
    def encrypt(self, key_id: str, plaintext: bytes, algorithm: str = None) -> Optional[bytes]:
        ...
        try:
            ...
            if metadata.key_type in ("aes", "chacha20"):
                ...
                aesgcm = Cipher(algorithms.AES(aes_key), modes.GCM(nonce)).encryptor()
    ...
    def key_exchange(
        self, key_id: str, peer_public_key: bytes, algorithm: str = None
    ) -> Optional[bytes]:
        """
    ...
    def wrap_key(
        self, key_id: str, wrapping_key_id: str = None, algorithm: str = None
...
if __name__ == "__main__":
    ...
    try:
        ...
        derived_key = key_manager.derive_key(
            key_id=key_id, context=b"key_derivation_test", key_size=32, algorithm="HKDF-SHA256"
        )
