...
class HSMInterface:
    ...
    def _init_azure_kv(self) -> None:
        ...
        if not HAS_AZURE_KV:
            raise ImportError("azure-identity and azure-keyvault-keys are required for Azure Key Vault support")
    ...
    def _init_gcp_kms(self) -> None:
        ...
        try:
            ...
            if self.config.get('create_key_ring', False):
                ...
                raise ValueError(f"Key ring {self._key_ring_path} not found and create_key_ring is False")
    ...
    def connect(self, **kwargs) -> bool:
        ...
        try:
            ...
            elif self.hsm_type in (HSMType.AWS_KMS, HSMType.AZURE_KEY_VAULT, HSMType.GCP_KMS, HSMType.SOFT_HSM):
    ...
    def _create_pkcs11_key(self, key_id: str, key_type: KeyType, key_size: int,
                          key_ops: List[KeyUsage], **kwargs) -> HSMKey:
        ...
        if key_type == KeyType.RSA:
            ...
            pub_attrs = self._session.getAttributeValue(pub_key, [
            ...
            return HSMKey(
                ...
                key_ops=key_ops,
                metadata={
                    'pkcs11_public_handle': pub_key,
                    'pkcs11_private_handle': priv_key
                }
            )
    ...
    def _create_aws_kms_key(self, key_id: str, key_type: KeyType, key_size: int,
                           key_ops: List[KeyUsage], **kwargs) -> HSMKey:
        ...
        try:
            ...
            return HSMKey(
                ...
                key_ops=key_ops,
                metadata={
                    'aws_key_arn': key_metadata['Arn'],
                    'aws_key_id': key_metadata['KeyId']
                }
            )
    ...
    def _create_azure_kv_key(self, key_id: str, key_type: KeyType, key_size: int,
                            key_ops: List[KeyUsage], **kwargs) -> HSMKey:
    ...
    def _create_gcp_kms_key(self, key_id: str, key_type: KeyType, key_size: int,
                           key_ops: List[KeyUsage], **kwargs) -> HSMKey:
        ...
        if 'policy' in kwargs:
            request['crypto_key']['version_destruction_behavior'] = \
                CryptoKey.VersionDestructionBehavior.DESTRUCTION_SCHEDULE
            
            # Set IAM policy
            policy = self._gcp_client.get_iam_policy(request={
                'resource': f"{key_ring_path}/cryptoKeys/{key_id}"
            })
        ...
        public_key = self._gcp_client.get_public_key(
            request={
                'name': f"{key.name}/cryptoKeyVersions/1"
            }
        )
    ...
    def _create_soft_hsm_key(self, key_id: str, key_type: KeyType, key_size: int,
                            key_ops: List[KeyUsage], **kwargs) -> HSMKey:
    ...
    def sign(self, key_id: str, data: bytes, 
             algorithm: str = 'SHA256', **kwargs) -> Optional[bytes]:
        """
    ...
    def _sign_pkcs11(self, key_id: str, data: bytes, 
                     algorithm: str, **kwargs) -> bytes:
    ...
    def _sign_aws_kms(self, key_id: str, data: bytes, 
                      algorithm: str, **kwargs) -> bytes:
    ...
    def _sign_azure_kv(self, key_id: str, data: bytes, 
                       algorithm: str, **kwargs) -> bytes:
        ...
        crypto_client = CryptographyClient(
            f"{self._key_client.vault_url}keys/{key_id}",
    ...
    def _sign_gcp_kms(self, key_id: str, data: bytes, 
                      algorithm: str, **kwargs) -> bytes:
    ...
    def _sign_soft_hsm(self, key_id: str, data: bytes, 
                       algorithm: str, **kwargs) -> bytes:
    ...
    def verify(self, key_id: str, data: bytes, signature: bytes,
               algorithm: str = 'SHA256', **kwargs) -> bool:
        """
    ...
    def _verify_pkcs11(self, key_id: str, data: bytes, signature: bytes,
                       algorithm: str, **kwargs) -> bool:
    ...
    def _verify_aws_kms(self, key_id: str, data: bytes, signature: bytes,
                        algorithm: str, **kwargs) -> bool:
    ...
    def _verify_azure_kv(self, key_id: str, data: bytes, signature: bytes,
                         algorithm: str, **kwargs) -> bool:
    ...
    def _verify_gcp_kms(self, key_id: str, data: bytes, signature: bytes,
                        algorithm: str, **kwargs) -> bool:
    ...
    def _verify_soft_hsm(self, key_id: str, data: bytes, signature: bytes,
                         algorithm: str, **kwargs) -> bool:
    ...
    def encrypt(self, key_id: str, plaintext: bytes, 
                algorithm: str = 'RSA-OAEP', **kwargs) -> Optional[bytes]:
        """
    ...
    def _encrypt_pkcs11(self, key_id: str, plaintext: bytes, 
                        algorithm: str, **kwargs) -> bytes:
        ...
        if algorithm.upper() == 'RSA-OAEP':
            mech = PyKCS11.RSA_PKCS_OAEP_Mechanism(
                PyKCS11.CKM_SHA256,
    ...
    def _encrypt_aws_kms(self, key_id: str, plaintext: bytes, 
                         algorithm: str, **kwargs) -> bytes:
    ...
    def _encrypt_azure_kv(self, key_id: str, plaintext: bytes, 
                          algorithm: str, **kwargs) -> bytes:
    ...
    def _encrypt_gcp_kms(self, key_id: str, plaintext: bytes, 
                         algorithm: str, **kwargs) -> bytes:
    ...
    def _encrypt_soft_hsm(self, key_id: str, plaintext: bytes, 
                          algorithm: str, **kwargs) -> bytes:
    ...
    def _decrypt_pkcs11(self, key_id: str, ciphertext: bytes, 
                        algorithm: str, **kwargs) -> bytes:
    ...
    def _decrypt_aws_kms(self, key_id: str, ciphertext: bytes, 
                         algorithm: str, **kwargs) -> bytes:
    ...
    def _decrypt_azure_kv(self, key_id: str, ciphertext: bytes, 
                          algorithm: str, **kwargs) -> bytes:
    ...
    def _decrypt_gcp_kms(self, key_id: str, ciphertext: bytes, 
                         algorithm: str, **kwargs) -> bytes:
    ...
    def _decrypt_soft_hsm(self, key_id: str, ciphertext: bytes, 
                          algorithm: str, **kwargs) -> bytes:
        ...
        for page in paginator.paginate():
            for key in page['Keys']:
                try:
                    ...
                    if kwargs.get('customer_managed_only', True) and \
            ...
            try:
                ...
                keys.append(HSMKey(
                    ...
                    metadata={
                        ...
                        'updated': key_props.updated_on.isoformat() if key_props.updated_on else None,
        ...
        for key in self._gcp_client.list_crypto_keys(
            ...
            try:
                ...
                if key.version_template.algorithm.name.startswith('RSA'):
                    key_type = KeyType.RSA
                    key_size = int(key.version_template.algorithm.name.split('_')[-2])  # e.g., RSA_SIGN_PKCS1_2048_SHA256 -> 2048
                ...
                keys.append(HSMKey(
                    ...
                    metadata={
                        ...
                        'next_rotation_time': key.next_rotation_time.ToDatetime().isoformat() if key.next_rotation_time else None,
    ...
    def _get_gcp_kms_key(self, key_id: str, **kwargs) -> Optional[HSMKey]:
        ...
        try:
            # Get the key
            key = self._gcp_client.get_crypto_key(
                name=f"{self._key_ring_path}/cryptoKeys/{key_id}"
            )
            ...
            if 'RSA' in primary_version.algorithm.name:
                key_type = KeyType.RSA
                key_size = int(primary_version.algorithm.name.split('_')[-2])  # e.g., RSA_SIGN_PKCS1_2048_SHA256 -> 2048
    ...
    def _get_soft_hsm_key(self, key_id: str, **kwargs) -> Optional[HSMKey]:
        ...
        return HSMKey(
            ...
            key_type=key_info['key_type'],
            key_size=key_info['key_size'],
            public_key=key_info.get('key'),  # In a real implementation, this would be the public key
            key_ops=key_info['key_ops'],
            metadata={
                'created_at': key_info['created_at'],
                'test': True
