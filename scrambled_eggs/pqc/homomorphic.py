...
class HomomorphicCrypto:
    ...
    def encrypt(
        self, plaintext: Union[int, float, List[Union[int, float]]], public_key: bytes
    ) -> bytes:
        """
    ...
    def decrypt(
        self, ciphertext: bytes, keypair: HomomorphicKeyPair
    ...
    def add(self, ciphertext1: bytes, ciphertext2: bytes, public_key: bytes) -> bytes:
        """
    ...
    def multiply(self, ciphertext: bytes, scalar: Union[int, float], public_key: bytes) -> bytes:
        """
    ...
    def _generate_paillier_keypair(self, key_size: int = 2048) -> HomomorphicKeyPair:
        ...
        # Create the key pair
        public_key = {"n": int(n), "g": int(g), "n_squared": int(n_sq)}

        secret_key = {"lambda": int(lambda_val), "mu": mu, "p": int(p), "q": int(q)}
    ...
    def _paillier_encrypt(
        self, plaintext: Union[int, float, List[Union[int, float]]], public_key_bytes: bytes
    ) -> bytes:
        ...
        # Handle lists/arrays
        if isinstance(plaintext, (list, tuple)) or (
            HAS_NUMPY and isinstance(plaintext, np.ndarray)
        ):
        ...
        result = {"c": int(ciphertext), "is_float": is_float, "scale": scale if is_float else 1}

        return json.dumps(result).encode("utf-8")

    def _paillier_encrypt_array(
        self, array: List[Union[int, float]], public_key_bytes: bytes
    ) -> bytes:
        """Encrypt an array of values using Paillier encryption."""
        encrypted = [
            json.loads(self._paillier_encrypt(x, public_key_bytes).decode("utf-8")) for x in array
        ]
        return json.dumps(encrypted).encode("utf-8")

    def _paillier_decrypt(
        self, ciphertext_bytes: bytes, keypair: HomomorphicKeyPair
    ) -> Union[int, float, List[Union[int, float]]]:
            ...
            if isinstance(data, list):
                return [
                    self._paillier_decrypt_single(
                        json.dumps(item).encode("utf-8"), n, n_sq, lambda_val, mu
                    )
                    for item in data
                ]
            else:
                return self._paillier_decrypt_single(ciphertext_bytes, n, n_sq, lambda_val, mu)
    ...
    def _paillier_decrypt_single(
        self, ciphertext_bytes: bytes, n: mpz, n_sq: mpz, lambda_val: mpz, mu: mpz
    ) -> Union[int, float]:
    ...
    def _paillier_raw_decrypt(self, c: mpz, n: mpz, n_sq: mpz, lambda_val: mpz, mu: mpz) -> int:
    ...
    def _paillier_add(
        self, ciphertext1_bytes: bytes, ciphertext2_bytes: bytes, public_key_bytes: bytes
    ) -> bytes:
        """Homomorphically add two Paillier ciphertexts."""
        # Deserialize the public key
        public_key = json.loads(public_key_bytes.decode("utf-8"))
        n_sq = mpz(public_key.get("n_squared", mpz(public_key["n"]) * mpz(public_key["n"])))
    ...
    def _paillier_multiply(
        self, ciphertext_bytes: bytes, scalar: Union[int, float], public_key_bytes: bytes
    ) -> bytes:
