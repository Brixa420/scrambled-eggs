...
class HybridCrypto:
    ...
    def generate_hybrid_kem_keypair(self, 
                                  key_type: KeyType = KeyType.EC,
                                  curve: ec.EllipticCurve = ec.SECP384R1(),
                                  rsa_key_size: int = 3072) -> Tuple[dict, dict]:
        """
    ...
    def encapsulate_hybrid(self, public_key_dict: dict) -> Tuple[bytes, dict]:
        ...
        if public_key_dict['kty'] == 'EC':
            ...
            pub_key = load_pem_public_key(
                public_key_dict['public_key'].encode('ascii')
            )
    ...
    def decapsulate_hybrid(self, encapsulation: dict, private_key_dict: dict) -> bytes:
        ...
        pq_shared_secret = self.kem.decapsulate(
            pq_ciphertext,
            bytes.fromhex(private_key_dict['pq_kem_private_key'])
        )
        ...
        if private_key_dict['kty'] == 'EC':
            ...
            private_key = load_pem_private_key(
                private_key_dict['private_key'].encode('ascii'),
                password=None
            )
            
            # Load the ephemeral public key
            pub_key = load_pem_public_key(
                encapsulation['ephemeral_public_key'].encode('ascii')
            )
    ...
    def sign_hybrid(self, message: bytes, private_key_dict: dict) -> dict:
        ...
        # Sign with ECDSA
        ecdsa_signature = private_key.sign(
            message,
        ...
        dilithium_sig = self.signer.sign(
            message,
            bytes.fromhex(private_key_dict['pq_sig_private_key'])
        )
    ...
    def verify_hybrid(self, message: bytes, signature_dict: dict, public_key_dict: dict) -> bool:
        ...
        try:
            ...
            public_key.verify(
                bytes.fromhex(signature_dict['ecdsa_signature']),
                message,
                ec.ECDSA(hashes.SHA384())
            )
            ecdsa_valid = True
    ...
    def decrypt_hybrid(self, 
        ...
        shared_secret = self.decapsulate_hybrid(
            encrypted_data['encapsulation'],
            private_key_dict
        )
