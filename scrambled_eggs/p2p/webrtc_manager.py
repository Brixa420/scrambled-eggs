...
class WebRTCManager:
    ...
    async def _handle_ice_candidate(self, from_id: str, candidate: dict):
        ...
        contact = self.contacts.get(from_id)
        if not contact or not contact.peer_connection:
            logger.warning(f"Received ICE candidate from unknown contact or no peer connection: {from_id}")
            return
    ...
    async def _create_peer_connection(self, contact: Contact) -> RTCPeerConnection:
        ...
        if not contact.data_channel:
            ...
            contact.data_channel.on("message", lambda msg: self._on_data_channel_message(contact.id, msg))
    ...
    def _encrypt_message(self, message: str, recipient_public_key_pem: bytes = None) -> dict:
        ...
        try:
            ...
            if recipient_public_key_pem:
                public_key = load_pem_public_key(recipient_public_key_pem, backend=default_backend())
        ...
        try:
            ...
            if 'counter' in encrypted_data:
                if encrypted_data['counter'] <= self.message_counter:
                    logger.warning(f"Possible replay attack detected: counter={encrypted_data['counter']}")
    ...
    # Public API
    async def add_contact(self, contact_id: str, name: str, public_key_pem: str, verify: bool = True) -> bool:
