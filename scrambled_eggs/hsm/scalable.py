...
class Shard:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'Shard':
        ...
        shard.metrics = data.get('metrics', {
            'keys': 0,
            'size_bytes': 0,
            'read_ops': 0,
            'write_ops': 0,
...
class ClusterManager:
    ...
    def _init_hash_ring(self) -> None:
        ...
        if not MMH3_AVAILABLE:
            raise ImportError("mmh3 is required for consistent hashing. Install with: pip install mmh3")
    ...
    async def rebalance_shards(self) -> None:
        ...
        available_nodes = [n for n in self.nodes.values() 
                         if n.status == NodeStatus.HEALTHY and not n.shard_id]
        ...
        for shard_id, shard in self.shards.items():
            ...
            if shard_id not in self.node_to_shard or self.node_to_shard[shard_id] != target_node.id:
                self.logger.info(f"Rebalancing: Assigning shard {shard_id} to node {target_node.id}")
    ...
    async def _assign_shard_to_node(self, shard_id: str, node_id: str) -> bool:
        ...
        self.logger.info(f"Assigned shard {shard_id} to node {node_id} "
