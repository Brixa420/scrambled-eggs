...
try:
    ...
    from azure.keyvault.keys import KeyClient, KeyVaultKey, KeyType as AzureKeyType
...
from .types import HSMKey, KeyType, KeyUsage, HSMType
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        if self.provider == CloudProvider.AWS_KMS.value:
            ...
            if not AZURE_AVAILABLE:
                raise ImportError("Azure Key Vault client not available. Install azure-identity and azure-keyvault-keys")
        ...
        try:
            ...
            if self.provider == CloudProvider.AWS_KMS.value:
                ...
                self._client.list_key_rings(
                    request={"parent": self._client.location_path(self.config.get('project_id'), self.config.get('location', 'global'))}
                )
        ...
        **kwargs
        ...
        try:
            ...
            response = self._client.encrypt(
                ...
                Plaintext=plaintext,
                EncryptionContext={
                    'aad': kwargs.get('aad', b'').hex()
                } if 'aad' in kwargs else {}
            )
    ...
    async def _aws_delete_key(self, key_id: str) -> bool:
        ...
        try:
            ...
            response = self._client.schedule_key_deletion(
                KeyId=key_id,
                PendingWindowInDays=7  # Minimum waiting period is 7 days
            )
            ...
            self.logger.info(f"Scheduled deletion of key {key_id}. Deletion date: {response['DeletionDate']}")
            return True
