...
async def test_hsm_async():
    ...
    # Test configuration
    config = {
        "cloud": {"provider": "aws_kms", "region": "us-east-1"},
        "pkcs11": {"library": "/path/to/pkcs11.so", "slot": 0, "pin": "1234"},
        "tpm": {"device": "/dev/tpm0"},
        "smartcard": {
            ...
            try:
                if await hsm.initialize():
                    ...
                    if key:
                        ...
                        if (
                            hasattr(key, "allowed_operations")
                            and KeyUsage.ENCRYPT in key.allowed_operations
                            and KeyUsage.DECRYPT in key.allowed_operations
                        ):
