...
class ScrambledEggsTray:
    ...
    def __init__(self, password: str, config_path: Optional[str] = None):
        ...
        if not HAS_GUI:
            raise RuntimeError(
                "GUI dependencies not installed. Install with: pip install pystray pillow"
            )
    ...
    def _create_icon(self):
        ...
        # Create menu items
        menu_items = [
            pystray.MenuItem("Status: Stopped", lambda: None, enabled=False),  # No action on click
            pystray.Menu.SEPARATOR,
            pystray.MenuItem("Start Protection", self._on_start_clicked, default=True),
            pystray.MenuItem("Stop Protection", self._on_stop_clicked, enabled=False),
            pystray.Menu.SEPARATOR,
            pystray.MenuItem("Protect File...", self._on_protect_file_clicked),
            pystray.MenuItem("Security Dashboard", self._on_dashboard_clicked),
            pystray.Menu.SEPARATOR,
        ...
        self.icon = pystray.Icon(
            "scrambled_eggs", image, "Scrambled Eggs Security", menu=pystray.Menu(*menu_items)
        )
    ...
    def _update_status_menu(self, status: Optional[str] = None):
        ...
        self.icon.menu = pystray.Menu(
            pystray.MenuItem(f"Status: {self.status.title()}", lambda: None, enabled=False),
    ...
    def _on_protect_file_clicked(self, icon, item):
        ...
        if self.icon:
            self.icon.notify("Select a file to protect", "File Protection")
    ...
    def _on_dashboard_clicked(self, icon, item):
        ...
        # For now, we'll just show a notification
        if self.icon:
            self.icon.notify("Opening Security Dashboard...", "Dashboard")
    ...
    def _on_settings_clicked(self, icon, item):
        ...
        # For now, we'll just show a notification
        if self.icon:
            self.icon.notify("Opening Settings...", "Settings")
