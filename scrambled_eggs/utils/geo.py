...
def get_routing_info() -> Dict[str, Any]:
    ...
    if routing_cfg.get("enabled", False):
        return {
            "enabled": True,
            "location": routing_cfg.get("coordinates", {"latitude": -82.8628, "longitude": 135.0}),
            "proxies": routing_cfg.get("proxies", []),
            "status": "active",
            "jurisdiction": "International Waters (Antarctica)",
        }

    return {"enabled": False, "status": "inactive", "jurisdiction": "Local Network"}
...
def obfuscate_network_metadata(metadata: Dict[str, Any]) -> Dict[str, Any]:
    ...
    coords = routing_cfg.get("coordinates", {"latitude": -82.8628, "longitude": 135.0})
