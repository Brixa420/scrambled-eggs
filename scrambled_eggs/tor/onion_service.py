...
class OnionService:
    ...
    async def start(self) -> str:
        ...
        try:
            ...
            service_config = {
                ...
                "ports": [
                    f"{self.config.virtual_port} {self.config.target_address}:{self.config.target_port}"
                ],
    ...
    async def get_service_descriptor(self) -> Optional[HiddenServiceDescriptor]:
        ...
        try:
            return await asyncio.to_thread(
                self.controller.get_hidden_service_descriptor, self.service_id
            )
        except Exception as e:
    ...
    async def add_port_mapping(
        self, virtual_port: int, target_address: str, target_port: int
    ) -> bool:
        """
    ...
    async def remove_port_mapping(self, virtual_port: int) -> bool:
        ...
        try:
            ...
            updated_ports = [
                f"{vport} {addr}" for vport, addr in current_ports.items() if vport != virtual_port
            ]
