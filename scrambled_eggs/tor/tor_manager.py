...
from .exceptions import TorConfigurationError, TorConnectionError, TorError, TorStartupError
...
class TorManager:
        ...
        try:
            if platform.system() == "Windows":
                result = subprocess.run(
                    ["where", "tor"], capture_output=True, text=True, check=False
                )
            else:
                result = subprocess.run(
                    ["which", "tor"], capture_output=True, text=True, check=False
                )
    ...
    async def start(self) -> None:
        ...
        try:
            ...
            # Create controller
            self.controller = await asyncio.to_thread(Controller.from_port, port=self.control_port)
            ...
            # Get Tor version
            try:
                version = await asyncio.to_thread(self.controller.get_version)
    ...
    async def stop(self) -> None:
        ...
        try:
            ...
            if self.controller:
                try:
                    await asyncio.to_thread(self.controller.signal, Signal.HALT)
                except Exception as e:
        ...
        try:
            for proc in psutil.process_iter(["pid", "name", "cmdline"]):
                try:
                    ...
                    if (
                        "tor" in proc.info["name"].lower() or "tor.exe" in proc.info["name"].lower()
                    ) and any(
                        "DataDirectory" in " ".join(cmd) for cmd in proc.info.get("cmdline", [])
                    ):

                        logger.warning(
                            f"Found orphaned Tor process (PID: {proc.pid}), terminating..."
                        )
                        proc.terminate()
                        try:
                            proc.wait(timeout=5)
    ...
    async def get_info(self, key: str) -> Optional[str]:
        ...
        try:
            return await asyncio.to_thread(self.controller.get_info, key)
        except Exception as e:
    ...
    async def get_circuit_info(self) -> List[Dict]:
        ...
        try:
            circuits = await asyncio.to_thread(self.controller.get_circuits)
            ...
            for circ in circuits:
                ...
                for node_fp, nick in circ.path:
                    try:
                        desc = await asyncio.to_thread(self.controller.get_network_status, node_fp)
                        ...
                        circuit_info["path"].append(
                            {"fingerprint": node_fp, "nickname": nick, "error": str(e)}
                        )
