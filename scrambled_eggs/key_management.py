...
class KeyManager:
    ...
    def __init__(self, storage_path: Optional[str] = None):
        ...
        """
        self.storage_path = (
            Path(storage_path) if storage_path else Path.home() / ".scrambled_eggs" / "keys"
        )
        ...
        try:
            ...
            for user_id, user in self._users.items():
                users_data[user_id] = {
                    ...
                    "private_key": (
                        base64.b64encode(user.private_key).decode("utf-8")
                        if user.private_key
                        else None
                    ),
    ...
    def share_key(
        self, key_id: str, from_user_id: str, to_user_id: str, permission: str = "read"
    ) -> bool:
        ...
        logger.info(
            f"Key {key_id} shared from {from_user_id} to {to_user_id} with {permission} permission"
        )
        return True
