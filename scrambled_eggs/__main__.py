...
def run_gui():
    ...
    try:
        ...
        logger.error(
            f"PySide6 is required for the GUI. Error: {e}. Install with: pip install PySide6"
        )
        return 1
    ...
    if args.user_command == "create":
        ...
        try:
            user = user_manager.create_user(
                username=args.username, password=password, is_admin=args.admin
            )
        ...
        if user_manager.reset_password(args.username, new_password, args.admin_username):
...
def main():
    ...
    try:
        ...
        logger.error(
            f"An error occurred: {e}", exc_info=args.verbose > 0 if "args" in locals() else False
        )
        return 1
